  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans">
    ...
    <div class="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 pb-6 border-b border-slate-800">
      <div class="flex items-center space-x-4">
        ...
        <div>
          <h1 class="text-2xl md:text-3xl font-black text-yellow-300 flex items-center space-x-2">
            <span>🛍️ Loja do Reino & Baú de Recompensas</span>
          </h1>
          <p class="text-xs md:text-sm text-slate-400">Gaste seu ouro conquistado nas tarefas diárias com equipamentos ou vales reais!</p>
        </div>
      </div>

      <!-- Saldo de Ouro -->
      <div v-if="activeCharacter" class="flex items-center space-x-3 bg-slate-900 border border-yellow-500/40 px-5 py-2.5 rounded-2xl shadow-lg shadow-yellow-500/10">
        <span class="text-2xl">🪙</span>
        <div>
          <p class="text-xs text-slate-400 font-bold">Saldo do Herói:</p>
          <p class="text-lg font-black text-yellow-300">{{ activeCharacter.gold }} Ouro</p>
        </div>
      </div>
    </div>
